...
def cleanup_routine():
	...
	subprocess.Popen("rm -rf demoCA/;rm newcert.pem;rm newreq.pem;rm src/program_junk/interfaces;rm src/html/*.exe;rm src/html/msf.exe;rm src/html/1msf.raw;rm src/html/2msf.raw;rm msf.exe;rm src/program_junk/*;rm src/html/unsigned/Signed_Update.jar;rm src/html/unsigned/mykeystore;rm src/html/index.html;rm src/html/1msf.exe;rm src/html/msf.exe;rm src/html/nix.bin;rm src/html/mac.bin;rm -rf src/program_junk/*;rm -rf src/program_junk/web_clone/*;rm src/html/1msf.exe;rm src/html/msf.exe;rm src/html/index.html;rm x", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True).wait()
	# Restore Original Java Applet
	subprocess.Popen("cp src/html/Signed_Update.jar.orig src/program_junk/Signed_Update.jar", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True).wait()
...
def show_banner(define_version,graphic):
  ...
  [---]          Codename: '""" + bcolors.YELLOW + """Rippin and Tearin""" + bcolors.BLUE + """'           [---]
  [---]       Report """ + bcolors.RED +"""bugs""" + bcolors.BLUE + """:"""+ bcolors.GREEN + """ davek@social-engineer.org  """ + bcolors.BLUE+"""   [---]
