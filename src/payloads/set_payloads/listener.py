...
# handle index error
	...
	try:
		...
		while 1:
			PrintWarning("Needs to be a port number!")
...
try:
	...
	PrintStatus("Crypto.Cipher library is installed. AES will be used for socket communication.")
	PrintStatus("All communications will leverage AES 256 and randomized cipher-key exchange.")
...
# if it isn't import then trigger error and turn off encryption
	...
	encryption = 0
        PrintWarning("Crypto.Cipher python module not detected. Disabling encryption.")
	PrintWarning("If you want encrypted communications download from here: http://ftp.dlitz.net/pub/dlitz/crypto/pycrypto/pycrypto-2.3.tar.gz")
	PrintWarning("Or if your on Ubuntu head over to: http://packages.ubuntu.com/search?keywords=python-crypto")
	PrintWarning("Or you can simply type apt-get install python-crypto or in Back|Track apt-get install python2.5-crypto")
...
try:
        ...
        if core_modules == True:
                log(error)
                PrintError("ERROR:Unable to bind to listener port, do you have something else listening?")
                sys.exit() #ExitSet()
...
def send_packet(message,conn,encryption):
	...
	# handle exceptions
		...
		PrintWarning("An exception occured. Handling it and keeping session alive. Error: " + str(e))
		pass
...
def handle_connection(conn,addr,encryption,operating_system):
    ...
    PrintStatus("Dropping into the Social-Engineer Toolkit Interactive Shell.")
	...
	PrintWarning("Looks like the session died. Dropping back into selection menu.")
	ReturnContinue()
	global count
	count = 2
	garbage1 = ""
    ...
    try:
		...
		while 1:
			...
			if data == "quit" or data == "exit" or data == "back":
				PrintWarning("Dropping back to list of victims.\n")
...
PrintStatus("The Social-Engineer Toolkit (SET) is listening on: 0.0.0.0:" + str(PORT))
					...
					while 1:
						try:
							...
							if choice == "quit" or choice == "exit" or choice == "back":
                                                                PrintStatus("Returning back to the main menu.")
								break
							if len(choice) != 0:
								choice = str(choice)
								print "[!] Invalid choice, please enter a valid number to interact with."
